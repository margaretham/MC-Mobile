...
class LoginActivity : AppCompatActivity() {
    ...
    private fun playAnimation() {
        ...
        val forgotPasswordTextView = ObjectAnimator.ofFloat(binding.forgotPasswordTextView, View.ALPHA, 1f).setDuration(300)
        val dontHaveAccountTextView = ObjectAnimator.ofFloat(binding.dontHaveAccountTextView, View.ALPHA, 1f).setDuration(300)

        AnimatorSet().apply {
            playSequentially(logoImageView, appNameTextView, emailEditText, passwordEditText, forgotPasswordTextView, loginButton, dontHaveAccountTextView)
            start()
        }
    }
    ...
    private fun setupAction() {
        ...
        binding.apply {
            ...
            loginButton.setOnClickListener {
                ...
                // Validate email
                if (!usernameEditText.text.toString().matches(checkSpaces.toRegex())) {
                    isError = true
                    usernameEditText.error = getString(R.string.no_whitespace)
                }
                ...
                if(!isError){
                    ...
                    if (isConnectedToApi) {
                        viewModel.loginUser(usernameEditText.text.toString(), passwordEditText.text.toString(), object : ResponseCallback {
